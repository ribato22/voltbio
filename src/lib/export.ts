...
export async function exportStaticSite(config: ProfileConfig): Promise<void> {
  ...
  if (config.settings.pwaEnabled) {
    const { profile, theme, settings } = config;
    const appName = profile.name || "My VoltBio";
    const shortName = settings.pwaShortName || appName.slice(0, 12);
    const themeColor = theme.colors.background;
    const bgColor = theme.colors.background;

    // Build icon array — use avatar as base64 data URI, or fallback
    const iconSrc = profile.avatar || getAvatarFallback(profile.name);
    const icons = [
    ...
    const manifest = JSON.stringify({
      ...
      description: config.seo.description || `${appName} — Link in Bio`,
    ...
    // Service Worker — caches index.html + avatar for offline
    const sw = `// VoltBio PWA Service Worker — Generated by VoltBio Export
const CACHE_NAME = 'voltbio-v1';
...
<head>
  ...
  ${settings.pwaEnabled ? `<!-- PWA -->
  <link rel="manifest" href="manifest.json" />
  <meta name="theme-color" content="${theme.colors.background}" />
  <meta name="apple-mobile-web-app-capable" content="yes" />
  <meta name="apple-mobile-web-app-status-bar-style" content="black-translucent" />
  <meta name="apple-mobile-web-app-title" content="${escapeHtml(settings.pwaShortName || profile.name?.slice(0, 12) || "VoltBio")}" />
  ${profile.avatar ? `<link rel="apple-touch-icon" href="${escapeHtml(profile.avatar)}" />` : ""}` : ""}
